...
SceneKit は WWDC25 で正式 deprecated のため不採用。RealityKit も検討したが、軽さ最優先の要件から Metal 直描画を選択。プレビュー用途では「単一パイプライン + 色別バッチ描画」で足りるためレンダラは小さく収まる（想定 500〜1000 行）。
...
flowchart LR
    subgraph app["IFC QuickLook.app (ホスト・ほぼ空)"]
        subgraph appex["PreviewExtension.appex"]
            VC[QLPreviewingController<br/>MTKView + オーバーレイ] --> MR[MetalRenderer<br/>バッチ統合・オービットカメラ]
...
| MetalRenderer | 色（マテリアル）別バッチ統合、単一パイプライン描画、オービットカメラ、プログレッシブ追記 | Metal / MetalKit |
...
4. メッシュを**色ごとに大きな GPU バッファへ追記統合** — ドローコール数を色数程度（通常 10〜50）に抑える
5. **最初のバッチが揃った時点（目標 1 秒以内）で描画開始**、残りは背景スレッドで流し込む（建物が徐々に現れる）
6. 完了後にバウンディングボックスでカメラフレーミング確定、オーバーレイに要素数・スキーマ版・読込時間を表示

## パフォーマンス・メモリ戦略

- パース完了後、web-ifc 側のトークンバッファを即解放。保持は **GPU 頂点バッファ + 色テーブルのみ**
...
- 実装難易度が高い（C++/Swift 連携・Metal 直描画）ため、実装は **Fable 5 モデルで行う**（subagent を使う場合も fable 指定）
